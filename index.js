...
async function run() {
  try {
    ...
    const earringsCollection = database.collection("earrings");
    ...
    app.get("/earrings", async (req, res) => {
      ...
      if (selected === "less600") {
        ...
        const result = await earringsCollection.find(filter).toArray();
        res.send(result);
      } else if (selected === "more500") {
        const filter = { price: { $gt: 500 } };
        const result = await earringsCollection.find(filter).toArray();
        res.send(result);
      } else if (selected === "more600") {
        const filter = { price: { $gt: 600 } };
        const result = await earringsCollection.find(filter).toArray();
        res.send(result);
      } else if (selected === "more700") {
        const filter = { price: { $gt: 700 } };
        const result = await earringsCollection.find(filter).toArray();
        res.send(result);
      } else {
        const allEarrings = await earringsCollection.find({}).toArray();
        res.send(allEarrings);
      }
    });

    app.get("/earring/:id", async (req, res) => {
      const earringID = req.params.id;
      const singleEarring = await earringsCollection
        .find({
          _id: ObjectId(earringID),
        })
        .toArray();

      res.send(singleEarring[0]);
    });

    app.post("/newEarring", async (req, res) => {
      const newEarringData = await req.body;
      await earringsCollection.insertOne(newEarringData);

      res.send(newEarringData);
    });
    ...
    app.get("/getProducts", async (req, res) => {
      const products = await earringsCollection.find({}).toArray();
    ...
    app.post("/deleteProducts", async (req, res) => {
      ...
      await earringsCollection.deleteOne({ _id: ObjectId(deleteReqId) });
